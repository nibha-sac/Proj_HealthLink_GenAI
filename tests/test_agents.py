...
class TestSymptomAgent:
    ...
    def test_symptom_agent_with_rag(self, mock_llm_client, mock_settings):
        ...
        with (
            patch("agents.symptom_agent.llm_generate") as mock_generate,
            ...
            # Run agent
            result = symptom_agent(
                user_input="I have chest pain", llm_client=mock_llm_client, settings=mock_settings, use_rag=True
            )
    ...
    def test_symptom_agent_error_handling(self, mock_llm_client, mock_settings):
        ...
        with patch("agents.symptom_agent.llm_generate") as mock_generate:
            ...
            result = symptom_agent(
                user_input="test input", llm_client=mock_llm_client, settings=mock_settings, use_rag=False
            )
...
class TestDoctorAgent:
    ...
    def test_doctor_agent_basic(self, mock_llm_client, mock_db_session, mock_settings):
        ...
        with (
            patch("agents.doctor_agent.llm_generate") as mock_generate,
            ...
            mock_generate.return_value = MockSpecialtyRec(
                recommended_specialty="General Practice", specialty_rationale="Test rationale", match_score=0.85
            )
