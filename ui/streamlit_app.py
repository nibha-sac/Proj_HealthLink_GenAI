...
# Page config
st.set_page_config(
    page_title="HealthLink - Smart Health Management", page_icon="🏥", layout="wide", initial_sidebar_state="expanded"
)
...
def display_symptom_analysis(symptom_data):
    ...
    for symptom in symptom_data["symptoms"]:
        duration_text = f"{' (Duration: ' + symptom['duration'] + ')' if symptom.get('duration') else ''}"
        st.markdown(
...
def main():
    ...
    st.markdown(
        "<p style='text-align: center; color: #666;'>Smart Health Management System</p>", unsafe_allow_html=True
    )
    ...
    with st.sidebar:
        ...
        st.write("HealthLink uses AI to analyze your symptoms and recommend appropriate healthcare providers.")
    ...
    with st.form("assessment_form"):
        user_input = st.text_area(
            ...
            placeholder=(
                "Example: I have had a severe headache for 3 days, along with fever and sensitivity to light..."
            ),
            height=150,
        )
        ...
        with col2:
            preferred_date = st.date_input(
                "Preferred appointment date (optional):", value=datetime.now() + timedelta(days=1)
            )
    ...
    if submit_button:
        ...
        with st.spinner("Analyzing your symptoms... This may take a moment."):
            try:
                ...
                # Call API
                response = requests.post(f"{API_BASE_URL}/assess", json=request_data, timeout=60)
                ...
                if response.status_code == 200:
                    ...
                    tab1, tab2, tab3, tab4 = st.tabs(["📋 Symptoms", "👨‍⚕️ Doctors", "📅 Scheduling", "📝 Summary"])
